...
const createTemperature = (obj) => {
  ...
  return new ConvertableValue(obj.value, WeatherUnits[obj.unitCode] || Units.Temperature.CELCIUS);
};

const createPressure = (obj) => {
  if (!obj.value) return null;
  return new ConvertableValue(obj.value, WeatherUnits[obj.unitCode]);
};
